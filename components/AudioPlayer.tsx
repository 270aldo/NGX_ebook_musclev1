...
import { generateNarrationAudio } from '../services/bonusApi';
...
const AudioPlayer: React.FC<AudioPlayerProps> = ({ readTime }) => {
  ...
  const [barHeights] = useState(() =>
    Array(16)
      .fill(0)
      .map(() => 4 + Math.floor(Math.random() * 16)),
  );
  ...
  const decodeAudioData = async (base64: string, mimeType: string, ctx: AudioContext): Promise<AudioBuffer> => {
  ...
  const fetchAudio = async () => {
    try {
      setIsLoading(true);
      const response = await generateNarrationAudio(CHAPTER_TEXT_PLAIN, conversationIdRef.current);

      if (!response.audioBase64) throw new Error('No audio data returned');

      const ctx = audioContext || initAudioContext();
      const buffer = await decodeAudioData(response.audioBase64, response.mimeType, ctx);
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between mb-2">
            <div>
              <p className="text-sm font-medium text-white/90">Narración Neural</p>
              <p className="text-xs text-white/40">Audio generado vía backend seguro</p>
            </div>
